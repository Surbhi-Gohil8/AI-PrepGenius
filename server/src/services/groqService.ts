...
import fs from 'fs';
import { ResumeData, GeneratedQuestion, Answer, ATSAnalysis } from '../types/dbTypes';
...
export const generateSuggestedAnswers = async (
  answers: Answer[],
  domain: string,
  difficulty: string,
  resumeData: ResumeData
): Promise<Record<string, { suggestedAnswer: string; keyPoints: string[] }>> => {
  if (answers.length === 0) return {};

  const qaBlock = answers
    .map(
      (a, i) =>
        `[${a.questionId}] Q${i + 1}: ${a.question}\nContext: ${a.context || 'N/A'}\nCandidate answered: ${a.transcript || '(no response)'}`
    )
    .join('\n\n');
...
Include one item per questionId listed above.`;
  ...
  try {
    ...
    const parsed = JSON.parse(content);
    const map: Record<string, { suggestedAnswer: string; keyPoints: string[] }> = {};
...
export const calculateResumeMatch = async (
  resumeData: ResumeData,
  answers: Answer[]
...
export const generateResumeTips = async (
  resumeData: ResumeData,
  answers: Answer[]
): Promise<string[]> => {
  try {
  ...
  try {
    ...
    return {
      ...
      issues: Array.isArray(parsed.issues)
        ? parsed.issues.slice(0, 6).map((i: any) => ({
