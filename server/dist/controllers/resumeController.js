...
const uploadResume = async (req, res) => {
    try {
        ...
        let rawText = '';
        try {
            rawText = await (0, resumeParserService_1.extractResumeText)(filePath, mimetype);
        }
        catch (parseError) {
        ...
        const parsedResumeData = await (0, groqService_1.parseResume)(rawText);
        // 3. Save parsed data to user document
        const updatedUser = await User_1.User.findByIdAndUpdate(req.user.id, { resumeData: parsedResumeData }, { new: true });
        if (!updatedUser) {
...
const deleteResume = async (req, res) => {
    try {
        ...
        const updatedUser = await User_1.User.findByIdAndUpdate(req.user.id, { resumeData: null }, { new: true });
        if (!updatedUser) {
            res.status(404).json({ message: 'User not found.' });
            return;
        }
